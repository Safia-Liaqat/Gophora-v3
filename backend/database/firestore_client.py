...
class FirestoreClient:
    ...
    def _initialize(self):
        ...
        try:
            ...
            if not firebase_admin._apps:
                ...
                # --- NEW CREDENTIALS HANDLING LOGIC ---
                
                # 1. Check for inline JSON (Set in Cloud Run environment variables)
                inline_json = os.getenv("FIREBASE_CREDENTIALS_JSON")
                
                if inline_json:
                    # Initialize directly from the JSON string content
                    try:
                        cred_dict = json.loads(inline_json)
                        cred = credentials.Certificate(cred_dict)
                        firebase_admin.initialize_app(cred, {'projectId': 'gophora-ai'})
                        logger.info("Firebase Admin SDK initialized from FIREBASE_CREDENTIALS_JSON.")
                    except Exception as e:
                        logger.critical(f"CRITICAL ERROR: Failed to parse and initialize Firebase from JSON string: {e}")
                        # Re-raise to crash the app and show the error in logs
                        raise 
                
                # 2. Check for Cloud Run ADC (The old logic is still good)
                elif os.getenv("K_SERVICE"): 
                    firebase_admin.initialize_app()
                    logger.info("Firebase Admin SDK initialized using Application Default Credentials (Cloud Run)")
                
                # 3. Fallback to Local/Other Configuration (Keep your original logic as a fallback)
                else:
                    # Your original logic: checks for b64, path, or falls back to ADC
                    cred_path = os.getenv("FIREBASE_CREDENTIALS_PATH", "./serviceAccount.json")

                    inline_b64 = os.getenv("FIREBASE_CREDENTIALS_BASE64")
                    
                    # Handle inline Base64 (e.g., from an environment variable)
                    if inline_b64:
                        # Write temporary file for initialization
                    ...
                    # Now, attempt to initialize from the file (local or runtime temp file)
                    if os.path.exists(cred_path):
                        cred = credentials.Certificate(cred_path)
                        firebase_admin.initialize_app(cred)
                        logger.info(f"Firebase Admin SDK initialized from file: {cred_path}")
                    else:
                        # Final safety net: fall back to ADC if no file is found locally
                        logger.warning(f"Firebase credentials not found. Falling back to ADC (Local).")
                        firebase_admin.initialize_app()
            ...
            # If initialization fails at any point, we log and re-raise
            logger.error(f"Failed to initialize Firestore: {e}")
            raise # Re-raise to crash the app and show the error in logs
    ...
    async def get_user(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if user_doc.exists:
                ...
                # ✅ FIXED: Add all ID field variations
                user_data['userId'] = user_doc.id  # For auth.py login function
                user_data['user_id'] = user_doc.id  # For other functions
                user_data['id'] = user_doc.id  # Keep existing
                return user_data
            return None
        except Exception as e:
    ...
    async def get_user_by_email(self, email: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for doc in docs:
                ...
                # ✅ FIXED: Add all ID field variations
                user_data['userId'] = doc.id  # For auth.py login function (line 86)
                user_data['user_id'] = doc.id  # For other functions
                user_data['id'] = doc.id  # Keep existing
                return user_data
            return None
        except Exception as e:
    ...
    async def store_refresh_token(self, user_id: str, token: str, expires_in_days: int = 7) -> bool:
    ...
    async def invalidate_refresh_token(self, user_id: str, token: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error invalidating refresh token for user {user_id}: {e}")
            return False
