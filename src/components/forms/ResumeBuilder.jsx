...
export default function ResumeBuilder() {
  ...
  const fetchResumes = async () => {
    try {
      ...
      setHasExistingResume(resumes.length > 0);
      console.log("ShowResume API response:", response.data);
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        if (hasExistingResume) {
          // Instead of creating a new one, update the existing one
          if (resumeList.length > 0) {
            const existingResumeId = resumeList[0].id;
            const response = await api.put(`/user/resumes/${existingResumeId}`, resumeData);
            console.log('Update existing resume response:', response.data);
            setSelectedResume(response.data.resume || resumeData);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10 flex gap-4 justify-center flex-wrap">
          ...
          <button
            onClick={() => {
              if (hasExistingResume && resumeList.length > 0) {
                // If user already has a resume, load it for editing
                const existingResume = resumeList[0];
                ...
                setFormData({
                  ...
                  languages: Array.isArray(existingResume.languages) ? existingResume.languages.join(', ') : existingResume.languages
                });
                setProfilePhoto(existingResume.profile_photo);
                setMode('edit');
              } else {
                // No resume exists, create new
                setMode('create');
                setSelectedResume(null);
          ...
          >
            <FiPlus className="inline mr-2" size={20} /> {hasExistingResume ? 'Edit Resume' : 'Create New Resume'}
          </button>
        </div>
            ...
            {loading ? (
              ...
              <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8 shadow-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {resumeList.slice(0, 1).map((resume) => ( // Only show first resume
                    <div
                      key={resume.id}
              ...
              <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-12 text-center">
                ...
                <h3 className="text-2xl font-bold text-white mb-3">No Resume Yet</h3>
                <p className="text-gray-400 mb-6">Create your professional resume to get started</p>
                <button
                ...
                >
                  <FiPlus size={20} /> Create Your Resume
                </button>
              </div>
            )}
          </div>
          ...
          <div className="max-w-5xl mx-auto">
            <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8 lg:p-12 shadow-2xl">
              <h2 className="text-3xl lg:text-4xl font-bold text-white mb-10 flex items-center gap-3">
                ...
                {mode === 'create' ? (hasExistingResume ? 'Update Your Resume' : 'Create New Resume') : 'Edit Resume'}
              </h2>
              ...
              <div className="pt-6 border-t border-white/10">
                ...
                >
                  {loading ? (
                    ...
                    mode === 'create' ? (hasExistingResume ? '💾 Update Resume' : '✨ Create Resume') : '💾 Update Resume'
                  )}
                </button>
              </div>
            </div>
